...
function filltable(array) {
  ...
  document.querySelectorAll(".accept-btn").forEach((button) => {
    button.addEventListener("click", function () {
      ...
      const donorBloodType = this.getAttribute("data-donor-bloodType");
      const donorBloodQuantity = this.getAttribute("data-donor-bloodQuantity");
      const centerEmail = localStorage.getItem("email"); // Use the correct email from localStorage
  
      console.log("Updating blood bank:", centerEmail, donorBloodType, donorBloodQuantity);
  
      // Update the donation request status
      updateSpecificDocument(donorEmail, "accepted");
  
      // Update the blood bank's blood type quantities
      updateBloodBank(centerEmail, donorBloodType, donorBloodQuantity);
    });
  });
  

async function updateBloodBank(centerEmail, bloodType, bloodQuantity) {
  try {
    // Ensure centerEmail is valid and not empty
    if (!centerEmail) {
      throw new Error("Invalid center email.");
    }

    const docRef = doc(db, "BloodBanks", centerEmail); // Use centerEmail as document ID

    // Update the specific blood type field by the specified amount
    await updateDoc(docRef, {
      [bloodType]: increment(parseInt(bloodQuantity)), // Ensure bloodQuantity is parsed as an integer
    });

    console.log(`Updated ${bloodType} by ${bloodQuantity} units for center ${centerEmail}`);
  } catch (error) {
